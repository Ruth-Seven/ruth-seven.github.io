---
title: python基础学习笔记-import和命名空间
...
Python 会智能地猜测一个变量是局部的还是全局的，它**假设任何在函数内赋值的变量都是局部的。**

因此，如果要给函数内**的全局变量赋值，必须使用 global 语句**。

`global VarName` 的表达式会告诉 Python， `VarName 1是一个全局变量，这样 Python 就不会在局部命名空间里寻找这个变量了。
